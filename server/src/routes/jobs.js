...
function enrichRow(r) {
    return {
        ...
        status: r.status,
        quantities: JSON.parse(r.quantities || "[]"),
...
router.get("/", requireAuth, (_req, res) => {
    ...
    res.json({ jobs: rows.map(enrichRow) });
});
...
router.post("/", requireAuth, (req, res) => {
    ...
    db.prepare(
        `INSERT INTO jobs (id, job_number, title, customer_id, customer_name, status, quantities, paper_type, binding_type, total_value, currency, priority, notes, due_date, payload_json, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(id, jobNumber, d.title, d.customerId, d.customerName, d.status, normalizeQuantities(d.quantities), d.paperType, d.bindingType, d.totalValue, d.currency, d.priority, d.notes, d.dueDate || "", JSON.stringify(d.payload || {}), now, now);

    writeAudit(actorId, "job.create", "job", id, { title: d.title, jobNumber });
    res.status(201).json({
        id, jobNumber, ...d,
        createdAt: now, updatedAt: now,
        quantities: Array.isArray(d.quantities) ? d.quantities : JSON.parse(d.quantities || "[]"),
    });
});
...
router.post("/:id/duplicate", requireAuth, (req, res) => {
    ...
    db.prepare(
        `INSERT INTO jobs (id, job_number, title, customer_id, customer_name, status, quantities, paper_type, binding_type, total_value, currency, priority, notes, due_date, payload_json, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, 'draft', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(id, jobNumber, `${original.title} (Copy)`, original.customer_id, original.customer_name, original.quantities, original.paper_type, original.binding_type, original.total_value, original.currency, original.priority, original.notes, original.due_date || "", original.payload_json, now, now);

    writeAudit(req.user?.sub || "system-local", "job.duplicate", "job", id, { originalId: req.params.id });
    res.status(201).json({ id, jobNumber, title: `${original.title} (Copy)`, createdAt: now });
});
