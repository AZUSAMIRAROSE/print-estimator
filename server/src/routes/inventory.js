...
const itemSchema = z.object({
    ...
    category: z.enum(["paper", "plates", "finishing", "packing", "ink", "chemicals", "consumables", "spare_parts", "other"]).default("other"),
...
router.get("/", requireAuth, (_req, res) => {
    ...
    res.json({ items: rows.map(enrichRow) });
});
...
router.post("/", requireAuth, (req, res) => {
    ...
    res.status(201).json(enrichRow({
        id, name: d.name, sku: d.sku, category: d.category, unit: d.unit,
        stock: d.stock, min_level: d.minLevel, cost_per_unit: d.costPerUnit,
        supplier: d.supplier, last_updated: now,
    }));
});
...
router.put("/:id", requireAuth, (req, res) => {
    ...
    res.json({ id: req.params.id, ...d, lastUpdated: now });
});
